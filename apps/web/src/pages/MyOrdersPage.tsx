  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    if (filter !== 'all') {
      result = result.filter(order => {
        ...
        if (filter === 'failed' && (order.status === 'failed' || order.status === 'timeout')) return true;
        return false;
      });
    }
            ...
            <div key={order.outTradeNo} className="bg-[#1A1A1A] rounded-2xl overflow-hidden">
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className="text-white font-semibold text-base">{order.assetName || order.subject}</div>
                ...
                <div className="flex justify-between items-center py-3 border-t border-white/10">
                  <div className="text-[#888888] text-sm">
                    {order.assetType === 'agent' ? '智能体' : order.assetType === 'capability' ? '能力包' : '商品'}
                  </div>
                  <div className="text-primary font-bold text-xl">¥{(order.totalAmount || order.amount || 0).toFixed(2)}</div>
                </div>
                ...
                <div className="border-t border-white/10 p-4 bg-[#151515]">
                  ...
                  {order.status === 'pending' && (
                    <div className="mt-4 pt-4 border-t border-white/10">
                      ...
                      >
                        {isRedirecting === order.outTradeNo || prepayMutation.isPending ? '跳转中...' : '去支付'}
                      </button>
                    </div>
                  )}
                  ...
                  {(order.status === 'failed' || order.status === 'timeout') && (
                    <div className="mt-4 pt-4 border-t border-white/10">
                      ...
                      >
                        {isRedirecting === order.outTradeNo || prepayMutation.isPending ? '跳转中...' : '重新支付'}
                      </button>
                    </div>
                  )}
